...
def upload(request):
    if request.method == 'GET':
        ...
        for i in range(0, row, 1):
              is_object_exists = EmployeeDetail.objects.filter(code = df.iloc[i][0]).exists()
              if is_object_exists is not True:
                employee = EmployeeDetail(
...
def chart(request):
  ...
  "chart": {
    "caption": "Apple's Annual Revenue",
    "subcaption": "for the year 2012-2019",
    "xaxisname": "Apple",
    "palettecolors": "#3be8b0",
    "plotgradientcolor": "#1aafd0",
    "yaxisname": "Annual Revenue 2012-2019 ",
    "numbersuffix": "K",
